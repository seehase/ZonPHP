...
$sqlref = "SELECT *
	...
	WHERE Month(Datum_Refer)='" . date("m", $_SESSION['sesdag']) . "' ";
...
$sql = "SELECT SUM( Geg_Dag ) AS gem, naam, 
    ...
    " WHERE Datum_Dag LIKE '" . date("Y-m-d", $_SESSION['sesdag']) . "%'  GROUP BY datumtijd, naam ".
    " ORDER BY datumtijd ASC";
//echo $sql;
$result = mysqli_query($con, $sql) or die("Query failed. dag " . mysqli_error($con));
if (mysqli_num_rows($result) == 0) {
    $datum = strftime("%d %B %Y", $_SESSION['sesdag']);
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if (!in_array($inverter_name, $sNaamSaveDatabase)){
            $sNaamSaveDatabase[] = $inverter_name;
        } ;
    }
...
$sqlmaxdag = "SELECT Datum_Maand, Geg_Maand
     ...
     DATE_FORMAT(Datum_Maand,'%m')='" . date('m', $_SESSION['sesdag']) . "'  GROUP BY  maand )AS maandelijks ON (month(" .
...
foreach ($sNaamSaveDatabase as $inverter_name)
{
    ...
    $newDate = mktime($hour, $minutes, 0, intval(date('m', $_SESSION['sesdag'])), intval(date('j', $_SESSION['sesdag'])), intval(date('Y', $_SESSION['sesdag'])));
...
//$maxmax =  $maxlink."--> ". $maxkwh. "kWh - ". $txt['peak'] .": " . number_format(max($agegevensdag_max), 0, ",", ".")
//    . "W" .'' . '';
//echo $maxmax;
...
var chart = new Highcharts.chart('mycontainer', Highcharts.merge(myoptions, {
