...
//###################################################################
    ...
    for ($hh = 0; $hh < $numberOfMonths; ++$hh) {
        ...
        //echo PLANT_NAMES[$k]," ",$hh," ",$h," ","<BR>";//controlestring
        //###################################################################
        //einde aanpassingen											   ##
        //###################################################################
        $sql = "
		Replace into tgeg_dag(IndexDag,Datum_Dag,Geg_Dag,kWh_Dag,Naam)
		select concat(t2.Datum_Dag,'" . PLANT_NAMES[$k] . "'),t2.Datum_Dag,
		concat(round(3600000*(t2.S0_" . ($k + $o) . " - t1.S0_" . ($k + $o) . ")/TIME_TO_SEC(TIMEDIFF( t2.Datum_Dag,t1.Datum_Dag )))), t2.S0_" . ($k + $o) . " - t1.S0_" . ($k + $o) . ", '" . PLANT_NAMES[$k] . "'
		from
...
select concat(DATE_FORMAT(Tijd_1,'%y-%m-%d'),'" . PLANT_NAMES[$k] . "') as IndexMaand, Datum_Dag, Result as Geg_Maand, '" . PLANT_NAMES[$k] . "' as Naam
from (
