...
//Output to PVOutput
//*************************************************
$sid= explode(",",$param['PVO_SYS_ID']);
for ($j = 0; $j < count($sNaamSaveDatabase); ++$j)
{
...
//echo $table_prefix;
$sql="SELECT Geg_Dag, Datum_Dag FROM ".$table_prefix."_dag where Naam = '".$sNaamSaveDatabase[$j]."' ORDER BY Datum_Dag DESC LIMIT 1";
//echo $sql;
//echo $sql;
...
$sql="SELECT Geg_Maand FROM ".$table_prefix."_maand where Naam = '".$sNaamSaveDatabase[$j]."' ORDER BY Datum_Maand DESC LIMIT 1";
